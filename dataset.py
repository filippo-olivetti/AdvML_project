...
class TextLoader:
  ...
  def __init__(self, data_dir):
    ...
    # a dictionary that provides default value int if an index is not present
  ...
  def load_data(self, data_dir):
    # create token_set that is a set of different characters
    #
  ...
  def split_data(self, data):
    ...
    for cat in data:
      ...
      cat_data = random.sample(cat_data, len(cat_data))

      train_ratio = int(len(cat_data) * 0.8)
      dev_ratio = int(len(cat_data) * 0.9)

      print(cat, len(data[cat]))

      test_split += [(dat,cat) for dat in cat_data[dev_ratio:]]
      train_split += [(dat,cat) for dat in cat_data[:train_ratio]]
      dev_split += [(dat,cat) for dat in cat_data[train_ratio:dev_ratio]]

      all_data += [(dat, cat) for dat in cat_data]

    train_split = random.sample(train_split, len(train_split))
